...
(:
    ...
    Alpheios edition
    Parameters:
        $a_cts the ctsURN element for the document, as parsed by ctsx:parseUrn
    Return Value:
        ...
        or
        B) if ctsx:parseUrn returned a dummy element because the urn was actually a text string, then it returns an
...
declare function tan:findDocs($a_cts)
{
  element docinfo
  {
    ...
    then
      ...
      let $docname :=
        fn:concat(
          $a_cts/fileInfo/basePath,
          "/alpheios-",
          $i,
          "-",
          $a_cts/fileInfo/alpheiosEditionId,
          ".xml"
        )
      return
        if (fn:doc-available($docname))
...
:)
declare function tan:matchMorph(
  $a_infl1 as node()*,
  $a_infl2 as node()*,
  $a_index as xs:int
) as xs:boolean*
{
  let $n := $a_infl1[$a_index]
  let $next := $a_infl1[$a_index + 1]
  ...
  return
    ...
    then
      ...
      then
        if ($next)
        then tan:matchMorph($a_infl1,$a_infl2,$a_index+1)
        else true()
      else
        false()
};
...
(:
    ...
    will be drawn from the morphology data document, which may have multiple possible lemmas for each form
:)
declare function tan:getWords(
  $a_docid as xs:string,
  $a_excludePofs as xs:boolean,
  $a_pofs as xs:string*
)
{
  let $cts := ctsx:parseUrn($a_docid)
  ...
  let $cacheDir := tan:idToCache($a_docid)
  let $cacheFileName :=
    fn:concat(
      if ($a_excludePofs) then 'not_' else 'all_',
      fn:string-join($a_pofs,'_'),
      '.vb.xml'
    )
  let $cachePath := fn:concat($collName, '/', $cacheDir,'/',$cacheFileName)
  return
    if (fn:doc-available($cachePath))
    then
      fn:doc($cachePath)
    else
      let $words :=
        ...
        then
            ...
            then
              fn:concat(
                ...
                (
                  ...
                  return
                    xs:string(
                      $tbDesc/tbd:table[@type eq "morphology"]
                             /tbd:category[@id eq 'pos']
                             /tbd:entry[tbd:long/text() = $p]
                             /tbd:short
                    )
                ),
                "|"),
          ...
          let $xsl := fn:doc('/db/xslt/treebank-to-lemma.xsl')
          let $params :=
            <parameters>
          ...
          return
             ctsi:xslt-transform($doc, $xsl, $params)//lemma
        ...
        :)
        then
          let $doc := fn:doc($docinfo/morph)
          let $lang := xs:string($doc/forms:forms/@*[local-name(.) = 'lang'])
          let $p_match :=
            if (fn:count($a_pofs) > 0)
            then fn:concat('^', fn:string-join($a_pofs,'|'), '$')
            else ""
          let $u_match :=
            ctsx:getUrnMatchString($ctsx:defaultInventory, $a_docid)
          ...
          let $lemmas :=
            ...
            then
              ...
              then
                $doc/forms:forms
                    /forms:inflection[forms:urn[matches(.,$u_match)]]
                    /forms:words
                    /forms:word
                    /forms:entry
                    /forms:dict[not(matches(forms:pofs,$p_match))]
              else
                $doc/forms:forms
                    /forms:inflection[forms:urn[matches(.,$u_match)]]
                    /forms:words
                    ...
                    /forms:entry
                    /forms:dict
                      [matches(forms:pofs,$p_match) or
                       matches(following-sibling::forms:infl/forms:pofs, $p_match)]
            else
              $doc/forms:forms
                  /forms:inflection[forms:urn[matches(.,$u_match)]]
                  /forms:words/forms:word
                  /forms:entry
                  /forms:dict
          let $all :=
            for $i in $lemmas
            ...
            let $sense :=
              if (fn:matches($i/forms:hdwd,"\d+$"))
              then fn:replace($hdwd, "^(.*?)(\d+)$", "$2")
              else ""
            let $lemma:=
              if (fn:matches($i/forms:hdwd, "\d+$"))
              then fn:replace($hdwd, "^(.*?)(\d+)$", "$1")
              else $hdwd
            let $form := $i/ancestor::forms:inflection/@form
            let $urns :=
              $i/parent::forms:entry
                /parent::forms:word
                /parent::forms:words
                /parent::forms:inflection
                /forms:urn[fn:matches(text(), $u_match)]
            let $count := fn:count($urns)
            return
              <lemma sense="{$sense}"
                     lang="{$lang}"
                     form="{$form}"
          ...
          return ctsi:xslt-transform(<lemmas>{$all}</lemmas>, $xsl, ())/*
          ...
          let $entries :=
            ...
            then
              ...
              then
                $parsed/word/entry
                  [dict
                    [fn:not(fn:matches(pofs, $p_match))] or
                     infl[fn:not(fn:matches(pofs, $p_match))]
                  ]
              else
                $parsed/word/entry
                  [fn:matches(dict/pofs, $p_match) or
                   fn:matches(infl/pofs, $p_match)]
      ...
      let $truncated  := for $i in fn:subsequence($words, 1, $tan:MAX_LEMMAS) return $i
      let $result :=
        <result count="{$returned}"
                total="{$total}"
                truncated="{$total - $returned}"
                treebank="{exists($docinfo/treebank)}"
                pmatch="{ if (count($a_pofs) > 0)
                          then fn:concat('^', fn:string-join($a_pofs, '|'), '$')
                          else ""        }"
                excludepofs="{$a_excludePofs}">
          <words>{for $i in $truncated order by $i/@lemma, $i/@form return $i}</words>
        </result>
      let $_ :=
        ctsi:document-store(
          fn:concat($collName, '/', $cacheDir),
          $cachePath,
          $result
        )
      return $result
...
declare function tan:getInflections($a_docid as xs:string, $a_pofs as xs:string*)
{
  ...
  let $u_match := ctsx:getUrnMatchString($ctsx:defaultInventory,$a_docid)
  let $forms :=
    if ($docinfo/morph)
    then
      ...
      return element inflection {
            ...
            <instances> {
              ...
              return
                ...
                then
                  let $reply := ctsx:getPassagePlus($ctsx:defaultInventory,$u)
    ...
    return
        <result count="{$returned}" total="{$total}" truncated="{$total - $returned}" treebank="{exists($docinfo/treebank)}">
          <forms>
          { fn:subsequence($forms, 1, $tan:MAX_FORMS) }
          </forms>
        </result>
};
...
:)
declare function tan:change-element-ns-deep(
  $a_nodes as node()*,
  $a_newns as xs:string,
  $a_prefix as xs:string
) as node()*
{
  for $node in $a_nodes
  return
...
declare function tan:change-element-ns-deep-with-prefix(
  $a_nodes as node()*,
  $a_newns as xs:string,
  $a_prefix as xs:string
) as node()*
  ...
  return
    ...
    then
      element
      {
        fn:QName(
          $a_newns,
          $a_prefix || fn:local-name($node)
        )
      }
      {
        $node/@*,
        tan:change-element-ns-deep-with-prefix(
          $node/node(),
          $a_newns,
          $a_prefix
        )
  ...
  let $word :=
    ...
    then
      ctsi:xslt-transform(
        ...
        $a_stripper,
        <parameters>
          <param name="e_in" value="{$i/@lemma}"/>
          <param name="e_toDrop" value="{$a_toDrop}"/>
        </parameters>
      )
    else $i/@lemma
  let $match :=
    ...
    then
      $a_vocab/tei:form[
        ...
        ctsi:xslt-transform(
          ...
          $a_stripper,
          <parameters>
            <param name="e_in" value="{text()}"/>
            <param name="e_toDrop" value="{$a_toDrop}"/>
          </parameters>
        ) = $word]
    else
      $a_vocab/tei:form[@type="lemma" and text() = $word]
  return
    if ($match)
    then
...
declare function tan:matchLemmas(
  $a_missed as xs:boolean,
  $a_words as node()*,
  $a_vocab as node()*,
  $a_stripper,
  $a_toDrop as xs:string
) as node()*
{
  if ($a_missed)
  then
    ...
    tan:matchLemmas($a_words, $a_vocab, $a_stripper, $a_toDrop)
};
...
{
  for $i in $a_vocab
  let $match := $a_words[@lemma = $i/tei:form[@type="lemma"]/text()]
  return $match
};
...
declare function tan:getMorph($a_text)
{
  ...
  let $wds := ctsi:xslt-transform($a_text, $xsl, $params)
  ...
  let $morph :=
    ...
    then
      ...
      then
        ...
        for $w in $wds//tei:wd
        let $buck_w:= ctsi:xslt-transform($w, $decode_xsl, ())
        let $url := replace($svcurl,'WORD',encode-for-uri($buck_w))
        let $morph := ctsi:http-get(xs:anyURI($url))
        ...
        return
          ctsi:xslt-transform($morph, $encode_xsl, ())//word
      ...
      then
        ...
        let $beta_w:= ctsi:xslt-transform($w, $decode_xsl,$params)
        (:let $beta_w := $w:)
        let $url := replace($svcurl,'WORD',encode-for-uri($beta_w))
        let $morph := ctsi:http-get(xs:anyURI($url))
        (: TODO: this is stripping the senses out before transforming to unicode
                 we need to isolate the sense in an attribute so it can be used for matching :)
        return ctsi:xslt-transform($morph, $encode_xsl, ())//word
...
declare function tan:get_OACMorph($a_nodes as node()*) as node()*
{
  ...
  return
    <forms:forms>{$morph_all}</forms:forms>
};
...
declare function tan:getTreebankRefs($a_cts as node(), $a_sentencesOnly as xs:boolean) as xs:string*
{
  ...
  let $cacheFile := tan:idToCache($a_cts/urn)
  let $cachePath := concat($collName, '/', $cacheFile, '.xml')
  let $_ :=
    if (fn:not(fn:doc-available($cachePath)))
    then
      let $nodes :=
        if ($a_cts/passageParts/rangePart)
        then
          ctsx:getPassagePlus($ctsx:defaultInventory,xs:string($a_cts/urn))
        else
          ctsx:getCitableText($ctsx:defaultInventory,xs:string($a_cts/urn))
      let $index :=
        for $node in ($nodes//text//wd,$nodes//tei:text//tei:wd)
          let $urn := ctsx:getUrnForNode($ctsx:defaultInventory,$a_cts,$node,'body',())
          ...
          let $allrefs :=
            if ($a_sentencesOnly)
            then
              fn:distinct-values(for $r in $tokenized order by $r return fn:substring-before($r,"-"))
            else
              fn:distinct-values(for $r in $tokenized order by $r return $r)
          for $ref in $allrefs
          return <ref urn="{$urn}">{$ref}</ref>
      return
        ctsi:document-store($collName, $cachePath, <refs>{$index}</refs>)
    else ()
...
{
  let $config := fn:doc('/db/xq/config/services.xml')
  return $config/services/morphology/service[@xml:lang=$a_lang]
};

(: cache name chops urn:cts and replaces : with _ and [] with # :)
declare function tan:idToCache($a_docid) as xs:string
{
  fn:replace(
    fn:replace(
      fn:replace($a_docid, 'urn:cts:', ''),
      ':',
      '_'
    ),
    '[\[\]]',
