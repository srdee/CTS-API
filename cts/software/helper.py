...
def instantiate(software, version, method, path, data_dir=None, target="./", user=None):
    """ Initiate the object
    ...
    if software.lower() == "existdb":
        return ExistDB(software=software, version=version, method=method, path=path, data_dir=data_dir, target=target, user=user)
    elif software.lower() == "basex":
        return BaseX(software=software, version=version, method=method, path=path, data_dir=data_dir, target=target, user=user)
    else:
